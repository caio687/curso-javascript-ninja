(function() {
  ...
  */
  ...
  const cpfsSujos = ["049-214 3421-1", "210.458.522-05", "735 500 794 - 22", "101.123-131x32"]
  

  const cpfsLimpos = cpfsSujos.map((cpfs, index) => {       
    return cleanCPF(cpfs)
  })

  console.log(cpfsLimpos)
  ...
  */
  ...
  const cpfFormatado = cpfsLimpos.map((cpfs, index) => {
    return cpfs = cpfs.replace(/(\d{3})(\d{3})(\d{3})(\d)/, (total, item1, item2, item3, item4) => {
  ...
  //Forma feita pelo Daciuk:
  /*
  const cpfFormatado = cpfsLimpos.map((cpfs, index) => {
    return cpfs = cpfs.replace(/(\d{3})(\d{3})(\d{3})(\d)/, '$1.$2.$3-$4')
  })
  ...
  */
  ...
  const juneOfJulyRegex = /ju(n|l)ho/g

  const texto = "Os meses de janeiro, junho e julho começam com a letra j."
  
  console.log(texto.match(juneOfJulyRegex))

  //Forma feita pelo Daciuk:
  //const juneOfJulyRegex = /ju[nl]ho/g
  ...
  */
  ...
  const htmlTag = "<div><section><blockquote>Texto <img /></blockquote></section></div>" 

  console.log(htmlTag.match(tagRegex))
  ...
  */
  ...
  const otherTagRegex = /(\<\w+\>)(\<\/\w+\>)/g 
  
  const htmlTag2 =  '<div><ul><li></li><li></li><li><span></span></li></ul></div>'

  console.log(htmlTag2.match(otherTagRegex))
  ...
  */
  ...
  const textoComTag = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>"

  //const textInsideTagRegex = /(<\w+>)(.+)(<\/\w+>)/g
  //const resultText = textoComTag.replace(textInsideTagRegex, (completo, tagAbertura, textoDentroDaTag, tagFechamento,) => {
  //return tagAbertura + 'O texto dentro da tag ' + tagAbertura + ' é ' + textoDentroDaTag + tagFechamento
  // })
  //console.log(resultText)

  //Forma resolvida pelo Daciuk
  const textInsideTagRegex = /(<(\w+)>)([^<]+)(<\/\w+>)/g

  const resultText = textoComTag.replace(textInsideTagRegex, '$1O texto dentro da tag "$2" é "$3"$4\n')
